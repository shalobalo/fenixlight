...
class shopProductsCollection
{
    ...
    protected function categoryPrepare($id, $auto_title = true)
    {
        ...
        if ($this->info['type'] == shopCategoryModel::TYPE_STATIC) {
            ...
            $count_price_sort = 'IF (p.`count` = 0 || p.price = 0, 1,0 ) AS sort';
            if ((empty($this->info['sort_products']) && !waRequest::get('sort')) || waRequest::get('sort') == 'sort') {
                $this->order_by = 'sort ASC, '.$alias.'.sort ASC';
                if( wa()->getEnv() == 'frontend' ) {
                    $this->order_by = 'p.pre_order DESC,' . $this->order_by;
                }
                $this->fields[] = $count_price_sort;
            } else {
                //##_## products with price 0 or count 0 insert to the end list
                if( wa()->getEnv() == 'frontend' ) {
                    $get_sort = waRequest::get('sort');
                    switch($get_sort) {
                        case 'rating' : $this->order_by = 'sort ASC, p.rating DESC'; $this->fields[] = $count_price_sort;break;
                        case 'total_sales' : $this->order_by = 'sort ASC, p.total_sales DESC'; $this->fields[] = $count_price_sort;break;
                        case 'price' : $this->order_by = 'sort ASC, p.price ASC'; $this->fields[] = $count_price_sort;break;
                        case 'name' : $this->order_by = 'p.name ASC';break;
                        case 'create_datetime' : $this->order_by = 'sort ASC, p.create_datetime DESC'; $this->fields[] = $count_price_sort;break;
                        case 'stock' : $this->order_by = 'sort ASC, count_null ASC, p.count DESC'; $this->fields[] = $count_price_sort;break;
                    }
                }
            }
        } else {
        ...
        foreach ($query_parts as $part) {
            ...
            if ($parts) {
                if ($parts[0] == 'category_id') {
                    ...
                    //##_##
                    $clear_search = preg_replace('/\W/', "", $parts[2]);
                    $search = new shopIndexSearch();

                    $word_ids = array();
                    if( strlen($clear_search) >= 3 ) {
                        $word_model = new shopSearchWordModel();
                        $where = array();
                        $where[] = "name LIKE '%". $clear_search ."%'";

                        if ($where) {
                            $sql = "SELECT id FROM shop_search_word WHERE ".implode(' OR ', $where);
                            $word_ids = $word_model->query($sql)->fetchAll(null, true);
                        }
