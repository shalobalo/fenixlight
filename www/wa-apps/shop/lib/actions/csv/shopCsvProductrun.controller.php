                ...
                if (!empty($this->data['options']['images'])) {
                    if (isset($product['images'])) {
                        ...
                        foreach ($product['images'] as & $image) {
                            //##_## Use original images for export
                            $path = shopProduct::getFolder($image['product_id']). "/". $image['product_id']."/" . "images/{$image['id']}.{$image['ext']}";
                            $image = 'http://'.ifempty($this->data['base_url'], 'localhost').wa()->getDataUrl($path, false, 'shop');
