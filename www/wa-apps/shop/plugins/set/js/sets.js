...
$.fn.shopSetPluginSets = function(options){
    ...
    .on('blur','.set-percent input', calcPrice)
    .on('keyup','.set-percent input', calcPrice)
	...
	function sortStop(e,ui){
		...
		$('.sets tbody tr',set).not('.last').each(function(i){
			arr[i] = $(this).data('set-item-id');
		})
	...
	function postEditItem(response,self){
		...
		if ( response.data.price ){
			...
			//var original_price = response.data.skus[ response.data.sku_id ].price; 
            //var price_incomplect        = Math.round(original_price - ((original_price*response.data.percent) / 100));             			
			//$('b',price_td).replaceWith('<b>' + price_incomplect + '</b>');

			/* percents */
			var input_percent = $('<input />').attr('type','text').addClass('bold numerical short');                                                
            input_percent.val(response.data.percent);            
            $('b',percent_td).replaceWith(input_percent);
    ...
    function calcPrice( ) {
        ...
        var original_price = $(this).parent().prev().prev().attr("data-price");
        price.val( Math.round(original_price - (original_price * percent / 100) ));        
    }
	...
	function setSaveItem(self){
		var tr = self.closest('tr'),
			...
			price_td = $('.set-price',tr),
			original_td = $('.original-price',tr),
			percent_td = $('.set-percent',tr),
			input_percent = $('input',percent_td),            
			sku_price = parseFloat(sku_td.next().data('price'));          
        
			var p = postData({
				item_id : tr.data('set-item-id'),
				price   : original_td.attr("data-price") ,
				sku_id  : $(':selected',select).val(),
                percent : input_percent.val(),
		...
		if ( $('loading',self).size() == 0 )
		{
			...
			$.post('?plugin=set&module=saveItem',p,function(response){
				price_td.html('<b>' + response.data.price + '</b>');				
                price_td.next().html('<b>' + input_percent.val() + '%</b>');
