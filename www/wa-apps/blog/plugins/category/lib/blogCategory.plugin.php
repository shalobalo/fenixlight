...
class blogCategoryPlugin extends blogPlugin
{
    ...
    public static function frontendSidebar($params)
    {
        $output = array();
        $category_id = isset($params['category'])?$params['category']:false;

        if($categories = blogCategory::getAll()){
            $output['sidebar'] = '<ul class="list-unstyled catalog">';
            $wa = wa();
            foreach ($categories as $category) {
                ...
                $output['sidebar'] .= <<<HTML
<li{$selected}>
<a href="{$category['link']}" title="{$category['name']}"><i class="fa fa-tags"></i>{$category['name']}</a>
</li>
HTML;
            }
            $output['sidebar'] .= '</ul>';
